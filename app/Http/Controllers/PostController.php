...
class PostController extends Controller
{
    ...
    # Thie method is going to retrieved all categories
    ...
    public function store(Request $request)
    {
        ...
        # Example: $request->category contains 3 categories --- Travel(1), Liifestyle(3), Movie(6)
        foreach ($request->category as $category_id) {
            $category_post[] = ['category_id' => $category_id];
        }
        $this->post->categoryPost()->createMany($category_post); // insert the user chosed categories data into the CategoryPost table (method from Post.php)
    ...
    {
        $post = $this->post->findOrFail($id); //search for the post
    ...
    public function update($id, Request $request)
    {
        ...
        if ($request->image)
        {
            $post->image = 'data:image/'. $request->image->extension(). ';base64'. baase64_encode(file_get_contents($request->image));
        }
        ...
        # Note: Use the relationship Post::categoryPoost() to select the records related post
        ...
        # 7. Return to the Show Post Page (to confilm the update is successful)
